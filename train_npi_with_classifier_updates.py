...
def train_adversarial_NPI(args): # train NPI and Classifiers in-tandem
    ...
    # print("############################################################")
    train_file_names = [str(pn)+"_CLEAN.pkl" for pn in range(num_pkls)]#os.listdir(train_file_path) # n8
    ...
    try:
        ...
        gpt2_with_npi = GPT2LMWithNPI.from_pretrained('gpt2') # n8 gpt2
        ...
        gpt2_tokenizer = GPT2Tokenizer.from_pretrained('gpt2') # n8 gpt2
        ...
        if args.content_classifier_optimizer_path is not None:
            content_class_optimizer = torch.load(args.content_classifier_optimizer_path)
        else:
        ...
        content_class_epoch_losses = []
        content_class_batch_losses = []
        ...
        for epoch in range(num_epochs):
            ...
            for file_num, train_file_name in enumerate(train_file_names):
                ...
                for batch, (orig_activ, real_label, target_label, data_inds) in enumerate(train_loader):
                    ...
                    orig_activ, real_label, target_label = orig_activ.cuda().float(), \
                                                            real_label.cuda().float(), \
                                                            target_label.cuda().float()
                    ...
                    if epoch >= HEAD_START_NUM:
                        ...
                        discrim_mask = np.array([False for _ in range(functional_batch_size)])
                        for j in range(functional_batch_size):
                            if real_label[j][2] == 1.:
                                discrim_mask[j] = True
                        orig_activ_discrim = orig_activ[discrim_mask]
                        functional_batch_size_discrim = orig_activ_discrim.shape[0]
                        pred_gpt2_outs_discrim = pred_gpt2_outs.detach()[discrim_mask]
                        ...
                        # labels # n8pe
                        y_real_GPT2 = torch.zeros(functional_batch_size_discrim).float().cuda() # 0 = real GPT2
                        y_fake_GPT2 = torch.ones(functional_batch_size_discrim).float().cuda() # 1 = fake GPT2
                        ...
                        real_gen_pred = generate_class_model(orig_activ_discrim) # n8pe
                        fake_gen_pred = generate_class_model(pred_gpt2_outs_discrim) # n8pe # n8SPEED
                        # loss
                    ...
                    if args.content_classifier_optimizer_path is not None:
                        ...
                        # UPDATE CONTENT CLASSIFIER WEIGHTS
                        ...
                        content_class_model.train()
                        content_class_optimizer.zero_grad()

                        # Determine output classifications
                        #vanilla_text_true_classification = torch.zeros(real_label.size()).cuda().float()
                        perturbed_text_true_classification = torch.zeros(real_label.size()).cuda().float()

                        for j in range(functional_batch_size):
                            vanilla_GPT2_text, perturbed_GPT2_text = training_text[j][1], training_text[j][3]

                            if not is_offensive(perturbed_GPT2_text):
                                perturbed_text_true_classification[j,2] = 1. # 1. means not offensive

                        vanilla_text_class_preds = content_class_model(orig_activ)
                        perturbed_text_class_preds = content_class_model(pred_gpt2_outs.detach())

                        vanilla_class_loss = content_class_objective(vanilla_text_class_preds.squeeze(), real_label[:,2].squeeze()) * 1e8
                        perturbed_class_loss = content_class_objective(perturbed_text_class_preds.squeeze(), perturbed_text_true_classification[:,2].squeeze())

                        content_class_loss = vanilla_class_loss + perturbed_class_loss
                        # collect for metadata?
                        content_class_batch_losses.append(content_class_loss.item())
                        content_class_loss.backward()
                        content_class_optimizer.step()
                ...
                if content_class_batch_losses: # n8
                    content_class_epoch_losses.append((sum(content_class_batch_losses) / float(len(content_class_batch_losses))))
                ...
                if epoch % test_freq == 0 and epoch >= HEAD_START_NUM:# and epoch >= 1: # AFTER TRAINING PERFORM ANY REQUIRED TESTS # n8 komya
                    ...
                    for test_batch,(test_x, test_t, test_y, test_inds) in enumerate(test_loader):
                        ...
                        test_x, test_t, test_y = test_x.cuda().float(), \
                                                 test_t.cuda().float(), \
                                                 test_y.cuda().float()
                        ...
                        if args.content_classifier_optimizer_path is not None:
                            content_class_model.eval()
                            test_perturbed_text_true_classification = torch.zeros(test_t.size()).cuda().float()
                            for j in range(batch_size):
                                test_vanilla_GPT2_text, test_perturbed_GPT2_text = test_text[j][1], test_text[j][3]
                                if not is_offensive(perturbed_GPT2_text):
                                    test_perturbed_text_true_classification[j,2] = 1. # 1. means not offensive
                            test_vanilla_text_class_preds = content_class_model(test_x)
                            test_perturbed_text_class_preds = content_class_model(test_gpt2_outs.detach())
                            test_vanilla_class_acc = my_accuracy(test_vanilla_text_class_preds.squeeze(), test_t[:,2].squeeze())
                            test_perturbed_class_acc = my_accuracy(test_perturbed_text_class_preds.squeeze(), test_perturbed_text_true_classification[:,2].squeeze())
                            test_content_class_acc = (test_vanilla_class_acc+test_perturbed_class_acc)/2.
                            content_class_test_batch_accuracies.append(test_content_class_acc)
                        ...
                        if file_num == len(train_file_names) - 1 and test_batch == 0: # n8+MD
                            ...
                            npi_sample_meta_data["testing data"]["epoch {}".format(epoch)] = \
                                                    {"style loss":test_style_loss.cpu().item(),
                                                      ...
                                                      "content class test accuracies":content_class_test_accuracies,
            ...
            if epoch % save_freq == 0 and epoch >= HEAD_START_NUM:# or epoch % 5 == 0: # n8
                ...
                print("Saving NPI Optimizer")
                out_path = save_file_path+"{}_npi_optimizer_epoch{}.bin".format(npi_type, epoch) # n8
                torch.save(npi_optimizer, out_path)
                print("Saving ContentClassifier Model")
                out_path = save_file_path+"{}_network_epoch{}.bin".format('ContentClassifier', epoch) # n8
                torch.save(content_class_model, out_path)
                print("Saving ContentClassifier Optimizer")
                out_path = save_file_path+"{}_optimizer_epoch{}.bin".format('ContentClassifier', epoch) # n8
                torch.save(content_class_optimizer, out_path)
                ...
                with open(out_path, 'wb') as outfile:
                    pkl.dump({#"false_test_losses": content_false_class_tests, 
                                "avg_epoch_losses": content_class_epoch_losses, # n8+MD
                ...
                print("Saving GenerationClassifier Optimizer")
                out_path = save_file_path+"{}_optimizer_epoch{}.bin".format('GenerationClassifier', epoch) # n8
                torch.save(generate_class_optimizer, out_path)
...
if __name__ == "__main__":
    ...
    """
    """
    --discrim_coeff 3 --style_coeff 10 --similarity_coeff 1 --test_freq 10 --npi_lr .000001 --class_lr .000001 --head_start_num 5
    """
    ...
    parser.add_argument("--content_classifier_optimizer_path",
                        default=None,
                        help="/path/to/optional_content_class_optimizer.bin")
    ...
    args.max_seq_len=15 # n8
    args.num_seq_iters=15 # n8 added
    ...
    if True: #for coeffs in coeffs_hyperparam_list:
        if True:#for layers in layers_hyperparam_list:
            ...
            args.save_file_path = orig_save_file_path + "params_discco{}_styco{}_simco{}_layers_{}_{}_npilr_{}_classlr_{}/".format(args.discrim_coeff, \
                                                                args.style_coeff, args.similarity_coeff, args.first_perturbation_index, \
                                                                args.second_perturbation_index, args.npi_lr, args.class_lr)
